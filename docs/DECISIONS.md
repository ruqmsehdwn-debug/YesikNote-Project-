...
- 날짜: 2026-07-20
- 상태: ACCEPTED · 문서 역할·범위 분리 원칙
- 결정자: Product Owner
- 최종 결정: 현재 PRD는 배포된 Owner Builder·MC MVP 구현 기준으로 유지하고, 현재 구현·파일럿 전 검토·장기 범위를 구분합니다.
- 문서 정리 원칙: 기존 문서를 먼저 삭제하거나 이동하지 않고, 새 문서가 기준 역할을 확보한 뒤 중복을 정리합니다.
- 후속 문서 후보 순서 상태: PROPOSED
...
- 범위 주의: 후보 순서는 제안이며 각 문서 작성은 별도 Batch 승인 대상입니다. 모든 문서를 제품 기능보다 먼저 작성한다는 뜻이 아닙니다.
...
- 변경 이력 결정: 최소 변경 이력은 파일럿 필수가 아닌 검토 후보이며 상세 데이터 구조는 이번 Batch에서 설계하지 않습니다.
...
- 관련 PR: [#2 Documentation Baseline Alignment](https://github.com/ruqmsehdwn-debug/YesikNote-Project-/pull/2)
